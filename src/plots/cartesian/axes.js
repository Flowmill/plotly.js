  ...
  for (
    ...
    if (ax.rangebreaks) {
      if (!axrev) {
        ...
        if (
          ax.maskBreaks(x) === BADNUM &&
          moveOutsideBreak(x, ax) >= maxRange
        ) {
          break;
        }
      }
...
function customFormat(tickformat, v, ax) {
  ...
  if (numeralMatch) {
    if (numeral) {
      ...
      console.warn('numeral() could not be found');
    }
  }
...
axes.drawLabels = function (gd, ax, opts) {
  ...
  if (
    ...
    fullLayout._insideTickLabelsAutorange[anchorAx._name + '.autorange'] =
      anchorAx.autorange;
