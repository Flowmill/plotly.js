...
var SECONDS_PER_HOUR = SECONDS_PER_MIN * 60;
var MILLISECONDS_PER_SEC = 1e3;
var SEC_PER_MILLISECONDS = 1 / MILLISECONDS_PER_SEC;
var MICROSECONDS_PER_SEC = 1e6;

/**
 * Converts the durationSec into a nicely formatted string that can be used
 * as a tick on a linear scale.
 *
...
function customFormat(tickformat, v, ax) {
  var numeralMatch = tickformat.match(/^numeral:(.+)$/);
  if (numeralMatch) {
    return numeral(v).format(numeralMatch[1]);
  }

  if (tickformat === 'duration') {
...
axes.makeTransFn = function (ax) {
  ...
  return axLetter === 'x'
    ? function (d) {
        return strTranslate(offset + ax.l2p(d.x), 0);
      }
    : function (d) {
        return strTranslate(0, offset + ax.l2p(d.x));
      };
};
...
axes.makeTransPeriodFn = function (ax) {
  ...
  return axLetter === 'x'
    ? function (d) {
        return strTranslate(offset + ax.l2p(getPeriodX(d)), 0);
      }
    : function (d) {
        return strTranslate(0, offset + ax.l2p(getPeriodX(d)));
      };
};
...
axes.makeLabelFns = function (ax, shift, angle) {
  var axLetter = ax._id.charAt(0);
  var ticksOnOutsideLabels =
    ax.tickson !== 'boundaries' && ax.ticks === 'outside';
  ...
  function fixLabelOverlaps() {
    ...
    if (
      ...
      if ((ax.tickson === 'boundaries' || ax.showdividers) && !opts.secondary) {
        ...
        var rotate90 =
          tickSpacing < maxFontSize * 2.5 || ax.type === 'multicategory';
...
function drawTitle(gd, ax) {
  ...
  if (ax.title.hasOwnProperty('standoff')) {
    ...
    if (ax.type === 'multicategory') {
      ...
      var offsetBase = 1.5;
      titleStandoff =
        10 + fontSize * offsetBase + (ax.linewidth ? ax.linewidth - 1 : 0);
    }
